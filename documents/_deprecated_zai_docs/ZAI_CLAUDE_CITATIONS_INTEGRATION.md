# Z.AI Web Search with Claude Citations Integration Guide
*Generated: November 20, 2025*

## Overview

This integration provides Z.AI GLM web search results formatted as **Claude's search_result blocks** with proper citations, enabling natural source attribution in Claude responses just like Claude's native web search.

## Architecture Pattern

```
User Query → Claude (via Anthropic interface) → Z.AI Web Search → Format as search_result blocks → Return to Claude with citations
                ↑
...
- **Function**: Converts Z.AI web search results into Claude's search_result format
- **Features**: Proper source attribution, citation formatting, natural integration

### **2. Claude Search Result Blocks Integration**
```json
{
  "type": "search_result",
...
Set up Claude Code with Z.AI as described in your documentation:
...
Add the Z.AI web search tool to your Claude Code setup:
...
# Add to Claude Code tools
claude_tools = [
    # ... existing tools
    search_tool.get_claude_tool_schema()
]
```

### **Step 3: Use in Claude Interactions**
```python
# When Claude needs web search, it can call the tool
...
# Result contains Claude-formatted search_result blocks
# Claude can now cite these results with proper attribution
```
...
Claude: "I'll search for current AI safety regulation developments..."
...
Claude: "Let me search for recent OpenAI safety announcements..."
...
Claude: "I'll search for technical information about quantum computing hardware..."
...
```python
{
  ...
  "description": "Z.AI GLM web search with Claude citation formatting...",
  "input_schema": {
    "type": "object",
    "properties": {
      "query": {
...
The tool returns results in Claude's search_result format:
...
If you're using Mini-Max as a proxy to Claude:

```python
from mini_agent.llm.anthropic_client import AnthropicClient
from mini_agent.tools.zai_web_search_with_citations import ZAIWebSearchWithCitationsTool

# Mini-Max provides Claude access
...
search_tool = ZAIWebSearchWithCitationsTool()
tools = [search_tool.get_claude_tool_schema()]

# Now Claude has web search with proper citations!
messages = [
    Message(role="user", content="Search for AI developments and provide citations")
]

response = await claude_client.generate(messages, tools=tools)
```

## Benefits

### **1. Natural Citations**
- Claude automatically includes citations when using search result information
- Proper source attribution matches Claude's native web search behavior
- Users get evidence-based responses with clear source links

### **2. Seamless Integration**
- Works with existing Claude Code setup using GLM Coding Plan
- Maintains conversation flow without separate web search steps
- Follows Claude's standard tool calling interface
...
- Compatible with Claude's citation system
...
# For Claude integration via Mini-Max
...
# Returns Claude-formatted search results with citations
```
...
2. **Claude Tool Registration**: Add the tool schema to your Claude Code setup
3. **Citation Testing**: Test that Claude properly cites search results
4. **Workflow Optimization**: Adjust parameters based on your use cases

---

**This solution provides Z.AI GLM web search with proper Claude citations, following your established integration pattern!**