...
class RedliningValidator:
    ...
    def validate(self):
        ...
        # First, check if there are any tracked changes by MiniMax-M2 to validate
        try:
            ...
            # Check for w:del or w:ins tags authored by MiniMax-M2
            del_elements = root.findall(".//w:del", self.namespaces)
            ins_elements = root.findall(".//w:ins", self.namespaces)

            # Filter to only include changes by MiniMax-M2
            minimax_del_elements = [
                elem
                for elem in del_elements
                if elem.get(f"{{{self.namespaces['w']}}}author") == "MiniMax-M2"
            ]
            minimax_ins_elements = [
                elem
                for elem in ins_elements
                if elem.get(f"{{{self.namespaces['w']}}}author") == "MiniMax-M2"
            ]

            # Redlining validation is only needed if tracked changes by MiniMax-M2 have been used.
            if not minimax_del_elements and not minimax_ins_elements:
                if self.verbose:
                    print("PASSED - No tracked changes by MiniMax-M2 found.")
                return True
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Remove MiniMax-M2's tracked changes from both documents
            self._remove_minimax_tracked_changes(original_root)
            self._remove_minimax_tracked_changes(modified_root)
            ...
            if self.verbose:
                print("PASSED - All changes by MiniMax-M2 are properly tracked")
            return True

    def _generate_detailed_diff(self, original_text, modified_text):
        """Generate detailed word-level differences using git word diff."""
        error_parts = [
            "FAILED - Document text doesn't match after removing MiniMax-M2's tracked changes",
            "",
            "Likely causes:",
    ...
    def _remove_minimax_tracked_changes(self, root):
        """Remove tracked changes authored by MiniMax-M2 from the XML root."""
        ...
        for parent in root.iter():
            to_remove = []
            for child in parent:
                if child.tag == ins_tag and child.get(author_attr) == "MiniMax-M2":
                    to_remove.append(child)
            for elem in to_remove:
                parent.remove(elem)

        # Unwrap content in w:del elements where author is "MiniMax-M2"
        ...
        for parent in root.iter():
            to_process = []
            for child in parent:
                if child.tag == del_tag and child.get(author_attr) == "MiniMax-M2":
