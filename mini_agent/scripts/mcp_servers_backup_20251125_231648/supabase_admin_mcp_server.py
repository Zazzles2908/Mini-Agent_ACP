...
try:
    ...
    # Silent failure - MCP protocol doesn't allow stdout errors
...
if not FASTMCP_AVAILABLE or not SUPABASE_AVAILABLE:
    # Silent failure - MCP protocol doesn't allow stdout errors
    # Dependencies will be validated separately via configuration script
    sys.exit(1)
...
def create_mcp_response(content: str, is_error: bool = False) -> dict:
    """
    Create proper MCP protocol response.
    
    Args:
        content: Response content (will be converted to proper MCP format)
        is_error: Whether this is an error response
        
    Returns:
        Proper MCP response dictionary
    """
    if is_error:
        return {
...
if not SUPABASE_URL or not SUPABASE_SERVICE_KEY:
    # Silent validation - MCP protocol requires silent failure
    # Configuration issues will be caught by separate validation script
    sys.exit(1)

try:
    supabase: Client = create_client(SUPABASE_URL, SUPABASE_SERVICE_KEY)
    # Database validation moved to individual tools to handle temporary outages
...
def execute_sql(request: QueryRequest) -> str:
    ...
    try:
        ...
        return create_mcp_response(json.dumps({
            "success": True,
            "data": result.data,
...
if __name__ == "__main__":
    # No stdout output allowed for MCP servers per JSON-RPC protocol
    # Debug messages should go to stderr only
    import sys
    if os.getenv("MCP_DEBUG", "false").lower() == "true":
        print("Starting Supabase Admin MCP Server...", flush=True, file=sys.stderr)
    mcp.run()
